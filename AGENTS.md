...
This pack is 18 skills and 13 agents. Any agent that imports it (a Grok Bot designer, a
...
1. First wake, or the user asks to install, set up, or onboard a fleet: **Wainwright
   Onboard**. **Wainwright Manager** is the orchestrator for that flow. It is not the
   Core / Straight Shooter persona acting alone.
...
5. The bot needs a schedule or event wake, and `profile.json` verification already passed:
...
6. A durable fact, preference, decision, or restated correction appears: **Grok Bot Memory**.
7. A fleet health or friction report is in view: **Fleet Healthcheck Hooks** (propose, then
...
it runs onboard and fleet orchestration. A designer uses the twelve personas two ways:
