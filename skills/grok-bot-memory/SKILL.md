...
Default to `log` when unsure between `profile` and `log`. Profile stays small.
...
check whether an existing entry says the opposite; `forget` the stale one, then write the new one.
